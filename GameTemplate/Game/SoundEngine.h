...
public:
	...
	* @brief	3Dサウンドソースを追加。
	*/
	void Add3DSoundSource(Game::SoundSource* s)
	{
	...
	* @brief	3Dサウンドソースを削除。
	*/
	void Remove3DSoundSource(Game::SoundSource* s)
	{
	...
	//サウンドソースをリストに追加
	void AddSoundSource(Game::SoundSource* s)
	{
		m_SoundSourceList.push_back(s);
	}

	//サウンドソースをリストから削除
	void RemoveSoundSource(Game::SoundSource* s)
	{
...
private:
	...
	std::list<Game::SoundSource*>	m_3dSoundSource;	//!<3Dサウンドのサウンドソース。
	X3DAUDIO_CONE	m_emitterCone;
	CWaveFileBank m_waveFileBank;					//!<波形データのバンク。

	std::list<Game::SoundSource*> m_SoundSourceList;//現在存在するすべてのサウンドソースのリスト
