...
namespace Util
{
	void TwoOnBackTriangle::MakeTriangles()
	{
		...
		m_backIndexBuffer->indices.push_back((*m_newpointArray)[1]);//残りの頂点の1つめ(TODO:後から変更、新頂点の2つ目になるかな?)

		m_backIndexBuffer->indices.push_back((*m_newpointArray)[0]);//新頂点1
		m_backIndexBuffer->indices.push_back(m_triangleData->vertexIndexes[m_diagonal]);//新頂点1の対角の頂点
		m_backIndexBuffer->indices.push_back(m_triangleData->vertexIndexes[BackLeftOf(m_diagonal)]);//残りの頂点の2つめ(TODO:後から変更、backIndexesのうちm_diagonalじゃない方になりそう)
	}

	uint32_t TwoOnBackTriangle::BackLeftOf(uint32_t diagonalIndex)
	{
